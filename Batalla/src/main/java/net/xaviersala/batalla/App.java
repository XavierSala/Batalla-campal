...
public class App extends GraphicsProgram {
   ...
   *          Número de soldats
   * @param fitxers
   *          Fitxers d'imatges a fer servir en aquest exèrcit
   * @param herois
   *          Fitxers d'imatges amb els herois
   * @param fort
   *          Soldat fort
   * @param rei1
   *          rei de l'exèrcit
   ...
   */
  public final Exercit creaExercit(final String nom, final int numSoldats,
  ...
  private void clicaPerComencar() {
    ...
    double centreX = (getWidth() - label.getWidth()) / 2;
    double centreY = (getHeight() + label.getAscent()) / 2;
    add(label, centreX, centreY);
    waitForClick();
    remove(label);
  }

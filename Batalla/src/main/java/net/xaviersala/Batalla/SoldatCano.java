...
public class SoldatCano extends SoldatGlobal {
    ...
    /**
     * Es crea el canó a partir de la seva imatge i de les
     * dimensions del camp de batalla.
     *
     * @param dibuix Imatge del canó
     * @param dibuixBala Imatge de la bala
     * @param llarg Dimensions del camp de batalla
     * @param alt Dimensions del camp de batalla
     */
    ...
    /**
     * El moviment del canó consisteix en decidir si ha de disparar
     * i moure totes les seves bales.
     *
     * @return no es mou.
     */
    @Override
    public final int mou() {
        ...
        if (tiraONo < cadenciaTret) {
            int posicioX = (int) ((int) getImatge().getX()
                    + getImatge().getWidth() / 2);
            int posicioY = (int) ((int) getImatge().getY()
                    + getImatge().getHeight() / 2);
            Bala projectil = new Bala(
                    new GImage(imatgeBala).getImage(),
                            posicioX, posicioY,
                    Aleatori.obtenir(dimensionsX),
